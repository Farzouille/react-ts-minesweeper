...
export function gameReducer(state: GameBoard, { type, payload }): GameBoard {
  switch (type) {
    ...
    case GameActions.UNCOVER_CELL:
      ...
      const gameWin = winCheck(newBoardUncovered, payload.row, payload.column, state.mines, state.gameSize, state.flagsLeft, 'left');
      return {
        ...state,
        board: (gameWin || newBoardUncovered[payload.row][payload.column].isMine) ? uncoverAllMines(state.board) : newBoardUncovered,
    ...
    case GameActions.RIGHT_CLICK_CELL:
      ...
      const gameWinFlag = winCheck(newBoard, payload.row, payload.column, state.mines, state.gameSize, state.flagsLeft, 'left');
      const gameOver = state.flagsLeft === 1 ? checkFlagsOnMines(state.board, payload.row, payload.column, state.mines) : false;
      return {
        ...state,
...
function initiateEmptyBoard(size: number): BoardMap {
  return Array.from(
    Array(size), () => new Array(size).fill({
      isMine: false,
      uncovered: false,
      connectingMines: 0,
      flagged: false
...
// TODO Too bruteforce ?
function addConnectingMineToAdjacentCells(
  board: BoardMap,
  x: number,
  y: number
): void {
